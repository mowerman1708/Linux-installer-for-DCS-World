...
Description: This script downloads the files needed for a DCS installation on
             GNU/Linux. Creates the wine prefix at your choice of location.
             Proceeds to run the DCS World installer.
...
script_dir = os.path.dirname(os.path.realpath(__file__))  # This script
install_dir = ""
wine_path = ""
dcs_url = "https://www.digitalcombatsimulator.com/upload/iblock/959/\
                    d33ul8g3arxnzc1ejgdaa8uev8gvmew2"
...
def convert_file_encoding(file_path, target_encoding='utf-8'):
    ...
    if current_encoding.lower() != target_encoding.lower():
        print(f"Converting file from {current_encoding} to\
              {target_encoding}...")
...
#================================================================#
#====== Functions to control the file downloads we need =========#
#================================================================#
        ...
        with open(dest, 'wb') as file:
            for chunk in response.iter_content(chunk_size=1024):
                ...
                if total_size > 0:  # stop possible divide by zero
                    ...
                    update_progress(progress_var, status_bar, progress,
                                    f"Downloaded {downloaded_size} of \
                                        {total_size} bytes...")
        update_progress(progress_var, status_bar, 100,
...
#================================================================#
#====== Functions to extract the Wine files we need     =========#
#================================================================#


def extract_wine_files(wine_file_path, runner_dir, progress_var, status_bar):
    # Check the file type
    file_type = subprocess.run(['file', wine_file_path], capture_output=True,
                               text=True)
    if "gzip compressed data" in file_type.stdout:
        # It's a .tar.gz file
        update_progress(progress_var, status_bar, 50,
                        "Extracting .tar.gz files...")
        try:
            subprocess.run(f"tar -xzvf {wine_file_path} -C {runner_dir}",
        ...
        try:
            subprocess.run(f"tar -xJvf {wine_file_path} -C {runner_dir}",
                           shell=True, check=True)
            update_progress(progress_var, status_bar, 100,
                            "Extraction completed successfully!")
            return True
        ...
        messagebox.showerror("File Type Error",
                             "The specified file is not a valid .tar.gz \
                                 or .tar.xz file.")
        return False

#================================================================#
#====== Functions to control the Status bar of the app  =========#
#================================================================#
...
#================================================================#
#====                 get_the_files()                        ====#
#==== This function first checks if a file exists and if not ====#
#====           it downloads the needed file.                ====#
#================================================================#

def get_the_files(progress_var, status_bar, root):
    os.makedirs(os.path.join(script_dir, 'tmp'), exist_ok=True)
    if not os.path.isfile(os.path.join(script_dir, 'tmp', wine_file)):
        if download_file(wine_git, os.path.join(script_dir, 'tmp', wine_file),
                         progress_var, status_bar, root):
            update_progress(progress_var, status_bar, 0,
                            f"{wine_file} downloaded successfully.")
    if not os.path.isfile(os.path.join(script_dir, 'tmp',
                                       dcs_installer)):  # check for installer
        if download_file(f"{dcs_url}/{dcs_installer}",
                         os.path.join(script_dir, 'tmp', dcs_installer),
                         progress_var, status_bar, root):
            update_progress(progress_var, status_bar, 0,
                            f"{dcs_installer} downloaded successfully.")

#================================================================#
#====            create_wine_prefix()                        ====#
#==== This function controls the creation of the wine prefix ====#
#================================================================#
...
def create_wine_prefix(progress_var, status_bar, root):
    ...
    while True:  # Start a loop to keep asking for the directory
        install_dir = filedialog.askdirectory(
                            title='Choose or add your DCS install location',
                            initialdir=os.path.expanduser("~"))
        if not install_dir:
            messagebox.showwarning(
                        "Warning", "No installation directory selected.")
            return
        wine_prefix_path = install_dir  # Set the wine prefix path
        if os.path.exists(wine_prefix_path) and not\
                messagebox.askyesno("Overwrite Existing Prefix",
                f"Wine prefix already exists at: {wine_prefix_path}\n\
                Do you want to overwrite it?"):
            continue  # This will go back to the start of the while loop
        break  # Exit the loop with a good install_dir
    ...
    try:
        update_progress(progress_var, status_bar, 10,
                        "Extracting Wine files...")
        # Step 1: Extract Wine files
        success = extract_wine_files(wine_file_path, runner_dir,
                                     progress_var, status_bar)
        if not success:
            update_progress(progress_var, status_bar, 100,
                            "Extraction failed.")
            return 1
        ...
        os.environ["wine_path"] = os.path.join(runner_dir, wine_directory,
                                               "bin")
        os.environ["WINEDLLOVERRIDES"] = "wbemprox=n;msdmo=n"
        # Install necessary components with winetricks
        update_progress(progress_var, status_bar, 75,
                        "Installing necessary components...")
        # Now run wintricks
        subprocess.run(["winetricks", "-q", "corefonts", "oleaut32",
                        "vcrun2017", "dxvk", "d3dcompiler_43",
                        "d3dcompiler_47", "d3dx9", "dotnetdesktop8", "win10"])
        update_progress(progress_var, status_bar, 100,
                        f"Wine prefix created successfully at \
                        {wine_prefix_path}")
    except subprocess.CalledProcessError as e:  #something failed
        messagebox.showerror("Error", f"Failed to set up Wine prefix: {e}")
        update_progress(progress_var, status_bar, 0,
                        "Error occurred during Wine prefix setup.")
        return 1
...
#================================================================#
#====            run_dcs_installer()                         ====#
#====   This function runs the DCS World install program     ====#
#================================================================#
...
def run_dcs_installer(wine_path, install_dir, progress_var, status_bar, root):
    ...
    if not os.path.isfile(dcs_installer_path):  #check if installer exists
        messagebox.showerror("Error", f"Installer file does not exist: \
                                {dcs_installer_path}")
        return
    try:
        update_progress(progress_var, status_bar, 0,
                        f"Installing DCS World at {install_dir}.")
        # run DCS installer
        ...
        update_progress(progress_var, status_bar, 100,
                        "DCS installation completed successfully.")
    except subprocess.CalledProcessError as e:
        # Log the error output to a file
        error_output = e.stderr.decode('utf-8', errors='replace')
        update_progress(progress_var, status_bar, 0, "DCS installation failed.")

    #routine checks for running wineserver and shuts it down
    try:
        subprocess.run([os.path.join(wine_path, 'wineserver'), '-q'],
                       check=True)  # Check if wineserver is running
    except subprocess.CalledProcessError:
        return  # Wineserver is not running, return without shutting down

    try:  # Wineserver is running, attempt to shut it down
        subprocess.run([os.path.join(wine_path, 'wineserver'), '-k'],
                       check=True)
    except subprocess.CalledProcessError as e:
        messagebox.showwarning("Warning",
                               f"Failed to shut down the Wine server. \
                                   Returned {e}")
    return

#================================================================#
#====                create_gui()                            ====#
#==== This function controls the creation of the main GUI    ====#
#================================================================#
...
def create_gui():
    ...
    tk.Label(title_frame, text="Welcome to GNU/Linux DCS Installer",
    ...
    busy_label = tk.Label(busy_frame, text="Please select:",
    ...
    def create_button_with_tooltip(text, command, tooltip_text):
        ...
        button.pack(pady=5)
        button.bind("<Enter>",
                    lambda e, text=tooltip_text: show_tooltip(e, text))
        button.bind("<Leave>", hide_tooltip)
        return button

    button1 = create_button_with_tooltip(
        "1.Download Necessary Files",
        lambda: run_task(run_get_the_files, button1),
        "Download required files for DCS.")
    button2 = create_button_with_tooltip(
        "2. Create Wine Prefix",
        lambda: run_task(run_create_wine_prefix, button2),
        "Set up a Wine prefix for DCS.")
    button3 = create_button_with_tooltip(
        "3. Install DCS",
        lambda: run_task(run_installer, button3),
        "Install the DCS game.")
    button_exit = create_button_with_tooltip("Exit", root.quit,
    ...
    status_bar = tk.Label(root, text="Ready", bd=1, relief=tk.SUNKEN,
                          anchor=tk.CENTER, font=("Arial", 11, "bold"))
    status_bar.pack(side=tk.BOTTOM, fill=tk.X)

    def run_task(task, button):
        global is_blinking
        button_exit.config(state=tk.DISABLED)   # Disable exit_button
    ...
    def task_wrapper(task, button):
        ...
        busy_label.config(text="Please Select", fg="green")  # Reset busy
        button_exit.config(state=tk.NORMAL)   # Re-enable exit_button

    def blink_busy_indicator():
        if is_blinking:  # Check if blinking should continue
            current_color = busy_label['foreground']
            busy_label['foreground'] = 'green'\
                if current_color == 'red' else 'red'
            root.after(750, blink_busy_indicator)  # blink speed

    def stop_blinking():
        global is_blinking
        is_blinking = False  # Set blinking flag to False
        busy_label['foreground'] = 'green'  # Ensure the label is green
    ...
    def run_installer():
        run_dcs_installer(wine_path, install_dir, progress_var,
                          status_bar, root)
